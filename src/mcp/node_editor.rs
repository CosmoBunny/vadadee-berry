...
pub fn kind_from_args(kind: &str, args: &Value) -> Result<GraphNodeKind, String> {
    ...
    match k.as_str() {
        ...
        "chroma_key" | "chroma" | "greenscreen" | "green_screen" => Ok(GraphNodeKind::ChromaKey),
        "apply_mask" | "matte" => Ok(GraphNodeKind::ApplyMask),
        "background_blur" | "bg_blur" | "bokeh_bg" => Ok(GraphNodeKind::BackgroundBlur),
        "regions_from_manual" | "manual_region" | "region_manual" => {
            Ok(GraphNodeKind::RegionsFromManual)
        }
        "privacy_blur" | "privacy" | "redact" => Ok(GraphNodeKind::PrivacyBlur),
        "detect_face" | "face_detect" | "faces" => Ok(GraphNodeKind::DetectFace),
        "track_motion" | "track" | "motion_track" => Ok(GraphNodeKind::TrackMotion),
        "union_image_2" | "union2" | "iu2" => Ok(GraphNodeKind::UnionImage2),
        "union_image_3" | "union3" | "iu3" => Ok(GraphNodeKind::UnionImage3),
        "union_image_5" | "union5" | "iu5" => Ok(GraphNodeKind::UnionImage5),
        "union_image_7" | "union7" | "iu7" => Ok(GraphNodeKind::UnionImage7),
...
pub fn kind_label(kind: &GraphNodeKind) -> String {
    match kind {
        ...
        GraphNodeKind::ChromaKey => "chroma_key".into(),
        GraphNodeKind::ApplyMask => "apply_mask".into(),
        GraphNodeKind::BackgroundBlur => "background_blur".into(),
        GraphNodeKind::RegionsFromManual => "regions_from_manual".into(),
        GraphNodeKind::PrivacyBlur => "privacy_blur".into(),
        GraphNodeKind::DetectFace => "detect_face".into(),
        GraphNodeKind::TrackMotion => "track_motion".into(),
        GraphNodeKind::UnionImage2 => "union_image_2".into(),
        GraphNodeKind::UnionImage3 => "union_image_3".into(),
        GraphNodeKind::UnionImage5 => "union_image_5".into(),
        GraphNodeKind::UnionImage7 => "union_image_7".into(),
